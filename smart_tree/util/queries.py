...
def pairwise_pts_to_nearest_tube(pts: np.array, tubes: List[Tube]):
    ...
    ab = collated_tube.b - collated_tube.a  # M x 3
    ap = pts - collated_tube.a  # N def pts_to_nearest_tube_keops(pts: np.array, tubes: List[Tube]):
    """Vectors from pt to the nearest tube"""
...
# GPU
def points_to_collated_tube_projections_gpu(pts: np.array, collated_tube: CollatedTube, device=torch.device("cuda")):
    ab = collated_tube.b - collated_tube.a  # M x 3

    ap = pts.unsqueeze(1) - collated_tube.a.unsqueeze(0)  # N x M x 3

    t = (torch.einsum("nmd,md->nm", ap, ab) / torch.einsum("md,md->m", ab, ab)).clip(0.0, 1.0)  # N x M
...
def pts_to_nearest_tube_gpu(pts: torch.tensor, tubes: List[Tube], device=torch.device("cuda")):
    """Vectors from pt to the nearest tube"""
    ...
    projections, t = points_to_collated_tube_projections_gpu(pts, collated_tube_gpu, device=torch.device("cuda"))  # N x M x 3
...
def skeleton_to_points(pcd, skeleton, chunk_size=4096, device="gpu"):
    ...
    for pts in tqdm(pts_chunks, desc="Labelling Chunks", leave=False):
        if device == "gpu":
            vectors, idxs, radiuses = pts_to_nearest_tube_gpu(pts, tubes)  # vector to nearest skeleton...
        else:
            vectors, idxs, radiuses = pts_to_nearest_tube(pts, tubes)  # vector to nearest skeleton...

        distances.append(np.sqrt(np.einsum("ij,ij->i", vectors, vectors)))  # could do on gpu but meh
